...
export class ObjectContext<T extends any> {
  ...
  public ensure$(handlers: EnsureHandlers<T>): Observable<ObjectState<T>> {
    try {
      ...
      if (this.currentState.state === State.loaded && !force) {
  ...
  private handleError(error: any, handlers: HandlersBase<T>, oneTime: Subscriber<ObjectState<T>>): void {
  ...
  private setFaulty(error: any): void {
    // @ts-ignore
    this.currentState.object = null;
